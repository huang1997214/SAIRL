...
class SchedulingEnv:
    ...
    def feedback(self, job_attrs, action, policyID):
        ...
        if policyID == 1:
            ...
            idleT = self.SAIRL_VM_events[0, action]
        ...
        if policyID == 1:
            ...
            self.SAIRL_events[0, job_id] = action
            self.SAIRL_events[2, job_id] = waitT
            self.SAIRL_events[1, job_id] = startT
            self.SAIRL_events[3, job_id] = durationT
            self.SAIRL_events[4, job_id] = leaveT
            self.SAIRL_events[5, job_id] = reward
            self.SAIRL_events[6, job_id] = real_length
            self.SAIRL_events[7, job_id] = success
            # update VM info
            self.SAIRL_VM_events[1, action] += 1
            self.SAIRL_VM_events[0, action] = new_idleT
        if policyID == 7:
            return my_reward
        return reward
    ...
    def get_accumulateRewards(self, policies, start, end):
        ...
        rewards[6] = sum(self.SAIRL_events[5, start:end])
    ...
    def get_FinishTimes(self, policies, start, end):
        ...
        finishT[6] = max(self.SAIRL_events[4, start:end])
    ...
    def get_executeTs(self, policies, start, end):
        ...
        executeTs[6] = np.mean(self.SAIRL_events[6, start:end])
    ...
    def get_waitTs(self, policies, start, end):
        ...
        waitTs[6] = np.mean(self.SAIRL_events[2, start:end])
    ...
    def get_responseTs(self, policies, start, end):
        ...
        respTs[6] = np.mean(self.SAIRL_events[3, start:end])
    ...
    def get_successTimes(self, policies, start, end):
        ...
        successT[6] = sum(self.SAIRL_events[7, start:end]) / (end - start)
    ...
    def get_rejectTimes(self, policies, start, end):
        ...
        reject[6] = sum(self.SAIRL_events[8, start:end])
    ...
    def get_totalRewards(self, policies, start):
        ...
        rewards[6] = sum(self.SAIRL_events[5, start:self.jobNum])
        return np.around(rewards, 2)
    ...
    def get_totalTimes(self, policies, start):
        ...
        finishT[6] = max(self.SAIRL_events[4, :]) - self.arrival_Times[start]
        return np.around(finishT, 2)
    ...
    def get_avgUtilitizationRate(self, policies, start):
        ...
        avgRate[6] = sum(self.SAIRL_events[6, start:self.jobNum]) / (
                    (max(self.SAIRL_events[4, :]) - self.arrival_Times[start]) * self.VMnum)
    ...
    def get_total_responseTs(self, policies, start):
        ...
        respTs[6] = np.mean(self.SAIRL_events[3, start:self.jobNum])
        return np.around(respTs, 3)
    ...
    def get_totalSuccess(self, policies, start):
        ...
        successT[6] = sum(self.SAIRL_events[7, start:self.jobNum]) / (self.jobNum - start + 1)
